...
- Este proyecto no maneja secretos propios (corre 100% en el cliente, sin backend). Aun así, nunca commitees credenciales de servicios de terceros.
...
- El proyecto **no tiene dependencias de runtime**: son HTML, CSS y JavaScript puros
  servidos de forma estática. La única carga externa es la tipografía (Google Fonts).
- Al no haber paquetes npm, no se requiere auditoría de dependencias ni lockfile.
- Dependabot (`.github/dependabot.yml`) se mantiene para vigilar acciones de GitHub
  Actions si se añaden workflows.
